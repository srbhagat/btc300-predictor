...
CACHE_FILE = os.path.join(DATA_DIR, "coindcx_btcusdt_15m_normalized_auto_final.csv")
...
def build_or_update_database(years_requested=2, force=False):
    """
    Correct CoinDCX history builder.

    The previous backward method often stopped at ~997 candles.
    This version uses forward time-window pagination:

    startTime = target start
    endTime   = startTime + 1000*15m
    repeat until now

    CoinDCX candle API supports startTime, endTime and limit=1000.
    """
    status = st.empty()
    progress = st.empty()
    ...
    # Auto-fix bad small cache such as 997 candles.
    ...
    # 1) Always fetch latest candles first.
    try:
    ...
    # 2) Forward build missing history from target_start to now.
    ...
    while cursor < now_ms:
        ...
        # If cache already covers this full window, skip it.
        ...
        # Do not stop on empty windows immediately. Some old ranges may be unavailable.
        # But if 20 consecutive windows are empty, continuing is not useful.
    ...
    # Keep only requested range to current time.
...
def analyze(df, price, direction, tp, sl, horizon, tol, session_match):
    """
    Adaptive historical matcher.

    The previous matcher could return 0 matches because all filters were applied at once.
    This version tries three fair mathematical levels:

    1. Strict match
    2. Relaxed match
    3. Wide match

    It does NOT force a trade. It only tries to get enough comparable historical examples.
    """
    latest = df.iloc[-1]
    ...
    for step in match_steps:
        ...
        # Minimum usable examples. Wide mode can be smaller but must not be zero.
...
st.title("BTCUSDT 300-Point Final Normalized Predictor")
st.caption("Final normalized version: auto-rebuild cache + normalized probabilities + fixed 300 TP / 400 SL.")
...
with st.sidebar:
    ...
    sl_points = st.number_input("Stop-loss points", value=400, min_value=50, step=50)
