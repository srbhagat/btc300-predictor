...
CACHE_FILE = os.path.join(DATA_DIR, "coindcx_btcusdt_15m_production_final.csv")
...
def analyze(df, price, direction, tp, sl, horizon, tol, session_match):
    ...
    match_steps = [
        {"name": "Strict", "price_tol": tol, "rsi_band": 12, "atr_low": 0.45, "atr_high": 1.65, "adx_band": 12, "vol_low": 0.35, "vol_high": 2.25, "trend": True, "major_trend": True, "session": session_match},
        {"name": "Relaxed", "price_tol": max(tol, 5.0), "rsi_band": 18, "atr_low": 0.30, "atr_high": 2.20, "adx_band": 18, "vol_low": 0.20, "vol_high": 3.50, "trend": True, "major_trend": False, "session": False},
        {"name": "Wide", "price_tol": max(tol, 8.0), "rsi_band": 25, "atr_low": 0.20, "atr_high": 3.00, "adx_band": 25, "vol_low": 0.10, "vol_high": 5.00, "trend": False, "major_trend": False, "session": False},
    ]
...
st.title("BTCUSDT 300-Point Production Final Predictor")
st.caption("Production final: working CoinDCX forward database + adaptive matching + fixed 300 TP + clear edge display.")
...
def edge_label(long_prob, short_prob):
...
def readable_grade(score, probability, decided):
    if decided < 20:
        return "VERY LOW SAMPLE"
    if score >= 80 and probability >= 68:
        return "A+ STRONG"
...
long_prob_display = float(long_r.get("weighted_probability", long_r["probability"]))
short_prob_display = float(short_r.get("weighted_probability", short_r["probability"]))
edge_gap_display = abs(long_prob_display - short_prob_display)
edge_text_display = edge_label(long_prob_display, short_prob_display)
lgrade_display = readable_grade(lscore, long_prob_display, long_r["decided"])
sgrade_display = readable_grade(sscore, short_prob_display, short_r["decided"])
