...
CACHE_FILE = os.path.join(DATA_DIR, "coindcx_btcusdt_15m_final_corrected.csv")
...
def add_indicators(df):
    ...
    # IMPORTANT:
    # CoinDCX candle API does not provide quote_volume/trades in this app.
    # Those columns are intentionally NaN, so never use df.dropna() on all columns.
    required_clean_cols = [
...
st.title("BTCUSDT 300-Point Final Corrected CoinDCX Predictor")
st.caption("Final corrected version: CoinDCX-only data, fixed 300 TP, stable indicators, strict NO TRADE filter.")
...
if len(df15_raw) < 5000:
    st.warning(f"Only {len(df15_raw):,} CoinDCX candles available. Signal will be stricter due to limited history.")
