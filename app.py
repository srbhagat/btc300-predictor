...
st.set_page_config(page_title="BTC 300 Final Hybrid Live", page_icon="₿", layout="wide")
...
def fetch_binance_latest_15m(start_ms=None, end_ms=None, limit=1500):
    """
    Latest Binance Futures 15m candles.
    This fills the gap between Binance Vision archive and the current moment.
    If blocked on Streamlit Cloud with HTTP 451, caller catches and falls back safely.
    """
    params = {"symbol": "BTCUSDT", "interval": "15m", "limit": int(min(limit, 1500))}
    ...
    headers = {"User-Agent": "Mozilla/5.0"}
    r = requests.get(BINANCE_FAPI_KLINES, params=params, headers=headers, timeout=20)
    r.raise_for_status()
    rows = r.json()
    if not rows:
        return pd.DataFrame(), "Binance latest API returned no rows"
    ...
    df["datetime"] = pd.to_datetime(df["open_time"].astype("int64"), unit="ms", utc=True).dt.tz_convert("Asia/Kolkata")
    df = df.drop_duplicates("open_time").sort_values("open_time").reset_index(drop=True)
    return df, f"Binance latest API loaded {len(df):,} candles"


def append_latest_binance_api(df):
    """
    Try to append missing candles from archive latest candle to current moment.
    This is the main fix for archive/live price drift.
    """
    if df.empty:
        return df, "No base archive data to update"

    step_ms = 15 * 60 * 1000
    last_ms = int(df["open_time"].max())
    start_ms = last_ms + step_ms
    now_ms = int(pd.Timestamp.now(tz="UTC").timestamp() * 1000)

    if start_ms >= now_ms - step_ms:
        return df, "Archive/latest data already close to current time"
    ...
    while start_ms < now_ms:
        end_ms = min(now_ms, start_ms + step_ms * 1500)
        try:
            dfl, msg = fetch_binance_latest_15m(start_ms=start_ms, end_ms=end_ms, limit=1500)
        except Exception as e:
            messages.append(f"Binance latest API unavailable: {e}")
            break
        ...
        frames.append(dfl)
        next_ms = int(dfl["open_time"].max()) + step_ms
        if next_ms <= start_ms:
            break
        start_ms = next_ms
    ...
    return out, " | ".join(messages[-3:]) if messages else "No latest API update needed"
...
def build_or_load_15m(years, live_price=None, force_rebuild=False):
    ...
    if valid and not force_rebuild:
        cached_updated, latest_msg = append_latest_binance_api(cached)
        if len(cached_updated) > len(cached):
            save_cache(cached_updated)
            status.success(f"15m database updated with latest API candles. Total {len(cached_updated):,}")
            st.info(f"Latest candle update: {latest_msg}")
            return cached_updated
    ...
    # FINAL LIVE UPDATE:
    # Archive gives deep history; Binance Futures API tries to fill missing candles to now.
    out, latest_msg = append_latest_binance_api(out)
    st.info(f"Latest candle update: {latest_msg}")
    ...
    if valid2:
        ...
        status.warning(f"Database built but warning remains: {reason2}")
        st.write("Recent archive/API logs:")
        st.write((logs + [latest_msg])[-20:])
...
def score_side(direction, res, latest, mtf, structure, support, resistance, price, tp, ctx):
    ...
    - 20% sample size
    - 15% trend/structure
    - 15% confirmation / blocking logic
    """
    reasons = []
    p = res.get("weighted_probability", res["probability"])
    d = res["decided"]

    # 1) Probability score out of 50
    if p >= 72:
    ...
    # 3) Trend/structure score out of 15
    trend_score = 0
    ...
    # 4) Confirmation score out of 15
    confirm_score = 0
    ...
    # Fixed 300 TP blocking penalties
    ...
    grade = grade_from_score(final_score, p, d)
    sample_label = sample_confidence_label(d)

    return final_score, reasons, grade, sample_label
...
st.title("BTCUSDT 300-Point Final Hybrid Live Predictor")
st.caption("Final hybrid live version: Binance archive + Binance latest API + CoinDCX live price.")
...
st.success(f"15m database ready with {len(df15_raw):,} candles. Latest data close: {df15_raw['close'].iloc[-1]:,.0f}")
st.caption(f"Latest data candle time: {pd.to_datetime(df15_raw['datetime'].max())}")
if not np.isnan(live_price):
    gap = abs(float(df15_raw['close'].iloc[-1]) - live_price) / live_price * 100
    if gap > 3:
        st.warning(f"Historical/latest data close differs from CoinDCX live price by {gap:.1f}%. Support/resistance fallback will use live price area.")
...
if archive_gap_pct > 3:
    st.warning(
        f"Historical/latest data close differs from CoinDCX live by {archive_gap_pct:.1f}%. "
...
lscore, lreasons, lgrade, lsample_label = score_side("LONG", long_r, latest, mtf, structure, support, resistance, price, tp_points, ctx)
sscore, sreasons, sgrade, ssample_label = score_side("SHORT", short_r, latest, mtf, structure, support, resistance, price, tp_points, ctx)
...
g1, g2 = st.columns(2)
g1.metric("Long sample confidence", lsample_label)
g2.metric("Short sample confidence", ssample_label)
