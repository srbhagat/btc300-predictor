...
st.set_page_config(page_title="BTC 300 Best CoinDCX Predictor", page_icon="₿", layout="wide")

# CoinDCX only. No Binance.
...
CACHE_FILE = os.path.join(DATA_DIR, "coindcx_btcusdt_15m_calibrated_final.csv")
...
def build_or_update_database(years_requested=2, force=False):
    ...
    cache = pd.DataFrame() if force else load_cache()
    frames = []

    # Always get latest candles first. This prevents stale-history problems.
    try:
        latest = fetch_coindcx_candles(limit=1000)
    except Exception as e:
        st.error(f"CoinDCX latest candles failed: {e}")
        latest = pd.DataFrame()
    ...
    # Backward pagination from earliest available candle until requested history reached.
    earliest = int(df["open_time"].min())
    batch = 0
    max_batches = int(years_requested * 365 * 96 / 1000) + 10

    while earliest > target_start_ms and batch < max_batches:
        end_ms = earliest - STEP_MS
        try:
            older = fetch_coindcx_candles(end_ms=end_ms, limit=1000)
        except Exception as e:
            status.warning(f"Older CoinDCX candles stopped: {e}")
            break

        batch += 1

        if older.empty:
            status.warning("CoinDCX returned no older candles. Using available history.")
            break

        df = pd.concat([df, older], ignore_index=True).drop_duplicates("open_time").sort_values("open_time").reset_index(drop=True)

        new_earliest = int(df["open_time"].min())
        ...
        progress.progress(
            min(1.0, batch / max_batches),
            text=f"CoinDCX history build: batch {batch}/{max_batches}, candles {len(df):,}, from {df['datetime'].min()}"
        )
        time.sleep(0.05)
    ...
    if age_minutes <= 120:
        status.success(f"CoinDCX database ready with {len(df):,} candles | latest {latest_dt}")
    else:
...
def grade_from_score(score, probability, decided):
    ...
    if score >= 80 and probability >= 68:
        return "A - Strong"
    if score >= 65 and probability >= 62:
        return "B - Good"
    if score >= 45 and probability >= 55:
        return "C - Watch"
    return "D - Weak"

def score_side(direction, res, latest, mtf, structure, support, resistance, price, tp, ctx):
    """
    Final calibrated quality score.
    ...
    - Trend/structure: 20
    - Confirmation: 20
    Then fixed-300TP blocking penalties are applied.
    """
    reasons = []
    p = res.get("weighted_probability", res["probability"])
    d = res["decided"]

    # Probability score out of 40
    if p >= 72:
        prob_score = 40; reasons.append("Excellent weighted probability")
    elif p >= 68:
        prob_score = 36; reasons.append("Very strong weighted probability")
    elif p >= 64:
        prob_score = 31; reasons.append("Strong weighted probability")
    elif p >= 60:
        prob_score = 25; reasons.append("Acceptable weighted probability")
    elif p >= 55:
        prob_score = 17; reasons.append("Mild weighted edge")
    elif p >= 52:
        prob_score = 10; reasons.append("Small weighted edge")
    elif p >= 50:
        prob_score = 6; reasons.append("Almost neutral probability")
    else:
        prob_score = 0; reasons.append("Probability too weak")

    # Sample score out of 20
    if d >= 250:
    ...
    if direction == "LONG" and ctx.get("bos") == "Bullish BOS":
        confirm_score += 5; reasons.append("Bullish BOS")
    if direction == "SHORT" and ctx.get("bos") == "Bearish BOS":
        confirm_score += 5; reasons.append("Bearish BOS")

    if direction == "LONG" and ctx.get("choch") == "Bullish CHOCH":
        confirm_score += 5; reasons.append("Bullish CHOCH")
    if direction == "SHORT" and ctx.get("choch") == "Bearish CHOCH":
        confirm_score += 5; reasons.append("Bearish CHOCH")

    if direction == "LONG" and ctx.get("sweep") == "Bullish sweep":
        confirm_score += 6; reasons.append("Bullish liquidity sweep")
    if direction == "SHORT" and ctx.get("sweep") == "Bearish sweep":
        confirm_score += 6; reasons.append("Bearish liquidity sweep")

    if ctx.get("volume_spike") == "Yes":
        confirm_score += 3; reasons.append("Volume expansion present")

    if direction == "LONG" and ctx.get("volume_zone") == "Near HV support":
        confirm_score += 5; reasons.append("Near high-volume support")
    if direction == "SHORT" and ctx.get("volume_zone") == "Near HV resistance":
        confirm_score += 5; reasons.append("Near high-volume resistance")

    confirm_score = min(20, confirm_score)

    raw_score = prob_score + sample_score + trend_score + confirm_score

    # Fixed 300 TP blocking logic remains strict.
    if direction == "LONG" and (resistance - price) < tp:
        raw_score -= 18; reasons.append("Resistance too close for fixed 300 TP")
    if direction == "SHORT" and (price - support) < tp:
        raw_score -= 18; reasons.append("Support too close for fixed 300 TP")
...
st.title("BTCUSDT 300-Point Final Calibrated Predictor")
st.caption("Final calibrated version: CoinDCX forward database + adaptive matching + balanced quality score + fixed 300 TP.")

with st.sidebar:
    st.header("Automatic settings")
    years = st.slider("CoinDCX history years requested", 1, 3, 2)
...
df15_raw = build_or_update_database(years, force)
...
if df15.empty:
    st.error("Not enough clean candle data after indicator calculation.")
    st.stop()
